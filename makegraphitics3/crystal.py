...
class Crystal(Sim):
    def __init__(self, molecule, lattice_dimensions, forcefield="OPLS"):
        ...
        if forcefield:
            ...
            self.forcefield = "OPLS"
        self.lattice = self.init_lattice()
        self.lattice_dimensions = self.determine_lattice(lattice_dimensions)
    ...
    def determine_lattice(self, lattice_dimensions):
        if not lattice_dimensions:
            ...
            return self.lattice.lattice_size_vdw(self.config["system"]["vdw_cutoff"])
        elif lattice_dimensions == "layers":
            return self.lattice.lattice_size_layers(
                self.config["system"]["vdw_cutoff"], self.config["system"]["N_layers"]
            )
        else:
    ...
    def generate_structure(self):
        ...
        self.molecule_labels = self.molecule.assign_molecules(self.lattice_dimensions)
        self.atom_labels = self.molecule.assign_atom_labels(self.lattice_dimensions)
